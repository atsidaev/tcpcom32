...
APU_DECLARE(void) apr_sha1_base64(const char *clear, int len, char *out)
{
	...
	int b64encode(apr_byte_t *src, unsigned int len, char *dest);
    ...
    /* private marker. */
    strncpy(out, APR_SHA1PW_ID, APR_SHA1PW_IDLEN + 1);


    /* SHA1 hash is always 20 chars */
    b64encode(digest, sizeof(digest), out + APR_SHA1PW_IDLEN);
